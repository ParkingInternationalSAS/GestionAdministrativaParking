...
function SendObject(myurl,parametros)
{
  ...
  console.log('SendObject: ' + JSON.stringify(parametros, null, 2));   
  console.log('SendObject: ' + myurl); 
  ...
  else
  {
    ...
    (
        ...
        (
            ...
            .append
            (
                $('<input>').attr('type', 'text').attr('id','obervaciones-' + id).attr('placeholder','Observaciones').attr('autocomplete','off').addClass('form-control').val(obervaciones)
            )
        )
        .append
        (
